...
DOCKER_NAME=${DOCKER_NAME:-noisy-relu-shift}
...
docker run \
    --runtime=nvidia \
    -v $(pwd):/noisy_relu_shift \
    -u $(id -u):$(id -g) \
    -w /noisy_relu_shift \
    --rm \
    -it \
    -e JUPYTER_DATA_DIR=/noisy_relu_shift/.jupyter \
    -p ${JUP_PORT}:${JUP_PORT} \
    --name ${DOCKER_NAME} \
    ${DOCKER_IMAGE} \
    jupyter notebook --no-browser --ip=0.0.0.0 --port=${JUP_PORT} --NotebookApp.token=${JUP_PASS} --notebook-dir='/noisy_relu_shift'
