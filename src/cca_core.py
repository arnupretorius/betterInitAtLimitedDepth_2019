...
#     try:
#       return_dict = get_cca_similarity(acts1, acts2, threshold, compute_dirns=False, verbose=False)
...
def compute_ccas_xy(sigma_xx, sigma_xy, sigma_yx, sigma_yy,
                 verbose=True):
  """Main cca computation function, takes in variances and crossvariances.
  This function takes in the covariances and cross covariances of X, Y,
  preprocesses them (removing small magnitudes) and outputs the raw results of
...
def get_cca_similarity(acts1, acts2, compute_coefs=True, threshold=0.98, compute_dirns=True,
                       verbose=False):
  """The main function for computing cca similarities.
  This function computes the cca similarity between two sets of activations,
  returning a dict with the cca coefficients, a few statistics of the cca
  ...
  ([u, s, v], _, _,
   _, _) = compute_ccas_xy(sigmaxx, sigmaxy, sigmayx, sigmayy,
                                  verbose=verbose)
...
def robust_cca_similarity(acts1, acts2, threshold=0.98, compute_dirns=True):
  ...
  for trial in range(num_cca_trials):
    try:
      ...
      acts2 = acts2 * 1e-1 + np.random.normal(size=acts1.shape) * epsilon
      if trial + 1 == num_cca_trials:
        raise ValueError("Number of CCA trails exceeded. No stable solution found.")
